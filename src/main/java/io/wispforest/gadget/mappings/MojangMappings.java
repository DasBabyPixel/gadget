...
public class MojangMappings extends LoadingMappings {
    ...
    protected void load(ProgressToast toast, MappingVisitor visitor) {
        try {
            ...
            Path mojPath = mappingsDir.resolve("mojmap-" + SharedConstants.getCurrentVersion().name() + ".tiny");
            ...
            for (int i = 0; i < versions.size(); i++) {
                ...
                if (GsonHelper.getAsString(version, "id").equals(SharedConstants.getCurrentVersion().id()))
                    chosenUrl = GsonHelper.getAsString(version, "url");
            }

            if (chosenUrl == null)
                throw new UnsupportedOperationException("Couldn't find version " + SharedConstants.getCurrentVersion().id() + " on Mojang's servers!");
